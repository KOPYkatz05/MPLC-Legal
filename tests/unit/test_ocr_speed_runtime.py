...
def test_ocr_runtime_mode_defaults_to_in_process(monkeypatch):
...
def test_extract_ocr_texts_defaults_to_in_process(monkeypatch):
    monkeypatch.delenv("MISSION_LEGAL_OCR_MODE", raising=False)
    monkeypatch.delenv("MISSION_LEGAL_OCR_IN_PROCESS", raising=False)
    ...
    monkeypatch.setattr(
        ...
        lambda image_paths, parent=None: [{"text": "in-process"}],
    )

    def fail_subprocess(image_paths):
        raise AssertionError("subprocess OCR should not run by default")
